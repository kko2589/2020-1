...
#define WIDTH	20
#define HEIGHT	10
void labelConponent(unsigned char img[HEIGHT][WIDTH],int x,int y,int label)
{
	if(x<0||y<0||x>=WIDTH||y>=HEIGHT)
		return;
	if(img[y][x]==9){
		img[y][x]=label;
		labelConponent(img,x-1,y,label);
		labelConponent(img,x+1,y,label);
		labelConponent(img,x,y-1,label);
		labelConponent(img,x,y+1,label);
	}
}
